...
function isTabRunning(wt: IssueWorktree | undefined, kind: OpenTab["kind"]): boolean {
  if (!wt) return false;
  return kind === "setup" ? wt.setup_status === "running" : wt.run_status === "running";
}

function channelId(wt: IssueWorktree | undefined, kind: OpenTab["kind"]): string | undefined {
  if (!wt) return undefined;
  return kind === "setup" ? wt.setup_task_id : wt.run_task_id;
}
...
// script); Run tabs are opened on demand from the scripts list. The tab content
// is the live streaming log for that channel.
export function WorktreeLogTabs() {
  const { openTabs, worktrees, activeKey, setActive, closeTab } = useWorktreeTabs();
  ...
  if (openTabs.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-1.5 border-l p-4 text-center text-[11px] text-muted-foreground">
        ...
        <span className="opacity-70">Run a script, or click Setup in the list above.</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <TabsList
        variant="line"
        className="h-8 w-full shrink-0 justify-start gap-1 overflow-x-auto rounded-none border-b px-2"
      >
        {openTabs.map((t) => {
          const wt = worktrees.find((w) => w.id === t.worktreeId);
          const key = tabKey(t.worktreeId, t.kind);
          const running = isTabRunning(wt, t.kind);
          return (
            <TabsTrigger
              key={key}
              value={key}
              className="h-[26px] flex-none gap-1.5 whitespace-nowrap px-2 text-[11px]"
            >
              <span className="truncate font-mono">{repoLabel(wt?.repo_url ?? "")}</span>
              <span className="text-muted-foreground/70">{t.kind === "setup" ? "setup" : "run"}</span>
        ...
        {openTabs.map((t) => {
          ...
          return (
            <TabsContent key={key} value={key} className="h-full min-h-0">
              <RunLog channelId={channelId(wt, t.kind)} />
            </TabsContent>
          );
        })}
      </div>
