...
type AgentTaskResponse struct {
	...
	// IssueBranch is the git branch the issue's worktrees check out (custom
	// branch_name, else the identifier-derived default). Daemon-claim only;
	// the daemon surfaces it to the agent (brief + MULTICA_ISSUE_BRANCH) so
	// code work continues on that branch instead of a fresh agent/* one.
