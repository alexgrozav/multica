...
INSERT INTO issue (
    ...
    $16, $17
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, branch_name
`
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.branch_name FROM issue i
WHERE i.workspace_id = $1
...
const listIssuesByBranchName = `-- name: ListIssuesByBranchName :many
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, branch_name FROM issue
WHERE workspace_id = $1 AND branch_name = $2 AND branch_name <> ''
ORDER BY created_at ASC
`

type ListIssuesByBranchNameParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	BranchName  string      `json:"branch_name"`
}
...
func (q *Queries) ListIssuesByBranchName(ctx context.Context, arg ListIssuesByBranchNameParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listIssuesByBranchName, arg.WorkspaceID, arg.BranchName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
